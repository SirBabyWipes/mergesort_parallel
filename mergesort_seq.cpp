...
namespace tasking{
void mergesort_task(int* arr, size_t l, size_t r, int* temp) {
    if (l >= r) return;

    size_t n = r - l + 1;
    size_t mid = (l + r) / 2;

    // Do small arrays sequentially
    ...
    // Spawn tasks
    tasking::taskstart([&]() {
        mergesort_task(arr, l, mid, temp);
    });

    tasking::taskstart([&]() {
        mergesort_task(arr, mid + 1, r, temp);
    });
...
void mergesort_parallel(int* arr, size_t n, int threads) {
    std::vector<int> temp(n);

    tasking::doinparallel([&]() {
        mergesort_task(arr, 0, n-1, temp.data());
    }, threads);
}
};
...
int main (int argc, char* argv[]) {
  ...
  if (argc == 3) {
    tasking::mergesort_parallel(arr.data(), n, atoi(argv[2]));
  }else {
    mergesort(arr.data(), 0, n-1, temp.data());
  }
